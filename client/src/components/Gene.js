...
export default function GeneTable(){
  ...
  const clickme = async (e) => {
    ...
    try {
      ...
      const response = await fetch("http://localhost:4020/api/v1/genecalculator", {
          method: "POST",
          headers: {"Content-Type": "application/json"},
          body: JSON.stringify(body)
      })
      const jsonData = await response.json()
      const bloodtype = JSON.stringify(jsonData.blood_type)
      const hcm = JSON.stringify(jsonData.hcm_result)
      const pdk = JSON.stringify(jsonData.pdk_result)
      setResult([JSON.stringify(jsonData.blood_type), JSON.stringify(jsonData.hcm_result), JSON.stringify(jsonData.pdk_result)])
    } catch(err) {
